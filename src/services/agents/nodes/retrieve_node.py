...
async def ainvoke_retrieve_step(
    ...
    if runtime.context.langfuse_enabled and runtime.context.trace:
        try:
            ...
            logfire.debug(f"Created Langfuse span for retrieval attempt {current_attempts + 1}")
        except Exception as e:
            logfire.warning(f"Failed to create span for retrieve node: {e}")

    # Check if max attempts reached
    if current_attempts >= max_attempts:
        logfire.warning(f"Max retrieval attempts ({max_attempts}) reached")
        fallback_msg = (
    ...
    logfire.debug(f"Created tool call for query: {question[:100]}...")

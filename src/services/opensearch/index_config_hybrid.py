...
ARXIV_PAPERS_CHUNKS_MAPPING = {
    "settings": {
        "number_of_shards": 1,  # Shards split index data for parallel storage & search
        "number_of_replicas": 0, # Replicas are backup copies for fault tolerance
        "index.knn": True,
        "index.knn.space_type": "cosinesimil",
        # settings for keyword searching
        "analysis": {
            "analyzer": {
    ...
    "mappings": {
        ...
        "properties": {
            ...
            "embedding": {
                ...
                "method": {
                    # HNSW solves this by:
                    # building a graph of vectors
                    # navigating only a small subset
                    # finding “almost nearest” neighbors very fast ✔️
                    "name": "hnsw",  # Hierarchical Navigable Small World .It is a fast approximate nearest neighbor (ANN) algorithm
                    "space_type": "cosinesimil",  # Cosine similarity
                    "engine": "nmslib", # This is the underlying library used for ANN search.
                    "parameters": { # These tune performance vs accuracy. Controls how carefully the index is built
                        "ef_construction": 512,  # Higher value = better recall, slower indexing
                        "m": 16,  # Number of connections each node keeps in the graph
                    },
                },
            },
            "authors": {
